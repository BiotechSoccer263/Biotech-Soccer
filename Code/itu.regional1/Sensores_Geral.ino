...
// Executa função 'linhas'
  ...
  //so vou passar a claucia de atacante perai
...
void linhas() {                                            // Define 'linhas' para...
  ...
  if (refletancia >= BrancoMin && refletancia <= BrancoMax) {  // Caso 'refletancia' seja maior ou igual que [BrancoMin] e 'refletancia' menor ou igual que [BrancoMax]...
    ...
    delay(350);   
    tras(150);                                                  // Movimentar atrás na velocidade [110]
    delay(250);                                                   // Esperar [300] milissegundos
  }
}
...
//dps testa o goleiro inverti o dribler, nao precisa, invertemos na propria ponte h 

// ficou ruim , ele ta pendendo pro lado esquerdo, e o problema de ir pra tras do nada ainda ta
...
void conduzir() {
  ...
  while (!chutou) {
    ...
    byte low = Wire.read();
    byte high = Wire.read();
    int leituraBruta = word(high, low) % 360;
    int bussolaCorrigida = (leituraBruta + OFFSET_BUSSOLA) % 360;

    int diferenca = (ANGULO_DO_GOL - bussolaCorrigida + 360) % 360;
    int diferencaAbs = diferenca > 180 ? 360 - diferenca : diferenca;
    ...
    if (frenteDist <= 80) {
      ...
      while (true) {
        ...
        Serial.print(F("🎯 Alinhamento Final | Bússola: "));
        Serial.print(bussolaCorrigida);
        Serial.print(F(" | Dif: "));
        Serial.println(diferencaAbs);

        if (diferencaAbs = TOLERANCIA_ANGULAR) {
          parar();
          Serial.println(F("✅ Alinhado no gol! CHUTANDO..."));
          chutar();
          break;
        ...
        if (diferenca <= 180) {
          rotacionar("Dir", VELOCIDADE_ROTACAO);
        } else {
          rotacionar("Esq", VELOCIDADE_ROTACAO);
        }
      }
    ...
    // --- Movimento com correção mínima ---
    if (diferencaAbs > 25) {
      // Corrigir apenas se erro for muito grande
      Serial.println(F("⚠️ Grande desalinhamento. Corrigindo..."));
      parar();
      if (diferenca <= 180) {
        rotacionar("Dir", VELOCIDADE_ROTACAO);
      ...
      // Dentro de faixa tolerável: apenas anda pra frente
      int difLateral = abs(direitaDist - esquerdaDist);
      if (difLateral <= 25) {
        Serial.println(F("✅ Indo para o gol com pequena correção..."));
        frente(190); // velocidade constante
      } else if (direitaDist < esquerdaDist) {
        Serial.println(F("⬅️ Ligeira correção para a esquerda..."));
        lateral("Esq", 100);
      } else {
