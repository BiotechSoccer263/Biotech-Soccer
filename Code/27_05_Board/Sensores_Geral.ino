...
void seguebola() {                                          
  ...
  if (ballDirecao == 0) {   // Caso a bola esteja fora do alcance...
    frente(120);            // Avança em linha reta em velocidade 120
  ...
  if (ballDirecao == 5 || ballIntensi >= IntensiMin) {  // Se a bola estiver à frente ou com intensidade mínima...
...
void linhas() {                                        
  ...
  if (UltraF.read() < 15 && ballIntensi < IntensiMin) {  // Se há obstáculo na frente e bola está distante...
    tras(110);                                           // Move para trás
    delay(300);                                          
  }
